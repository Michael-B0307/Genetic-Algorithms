...
# Deceptive fitness function to calculate the fitness of a genome (counting of 1's in the string),
# except when there are no 1s present. In this case, the fitness will be 2 * (length of the solution).
def fitness(genome):
    """Calculate the fitness of a genome.
    Fitness is measured as the count of 1's in the genome. If no 1s are present,
    the fitness is set to twice the length of the genome, to represent a deceptive fitness landscape.

    :param genome: A list of bits (0s and 1s) representing the genome.

    :return: The fitness value as an integer.
    """
    ones_count = sum(genome)
    return ones_count if ones_count > 0 else 2 * GENOME_LENGTH
...
def main():
    ...
    with open('fitness_history_deceptive.csv', 'w') as file:
